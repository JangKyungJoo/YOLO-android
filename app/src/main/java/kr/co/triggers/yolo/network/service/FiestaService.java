...
import kr.co.triggers.yolo.domain.impl.Artist;
import kr.co.triggers.yolo.domain.impl.Fiesta;
...
public interface FiestaService {
    ...
    Observable<Response<Artist>> lineup(@Path("id") long id);
    ...
    @GET("/fiesta/{id}/react")
    Observable<Void> react(@Path("id") long id, @Query("status") String status);
}
